if __name__=='__main__':
    ...
    def get_model_invariant(train_loader1, train_loader2, len_embedding, abstract_len_embedding, trial_no):    
        ...
        ######################################################################################
            ...
            if epoch % 1 == 0:
                with open('/curdir/v3/logger_' + exp_name + '_' + str(trial_no) + '.log', 'a+') as file1:
                    file1.writelines(f'epoch: {epoch}, total_loss: {total_loss:.6f}\n\n')

        return model_invariant

    df1 = pd.read_pickle(f'/curdir/v3/cathub_df.pickle')
    X1 = df1.iloc[:, :-1].values
    y1 = df1['energy'].values
    df2 = pd.read_pickle(f'/curdir/v3/ocp_df.pickle')
    ...
    for trial_no in range(ntrial):
        ...
        ## save the model
        model_path = '/curdir/v3/model_invariant_' + str(exp_name) + '_' + str(trial_no) + '.pt'
