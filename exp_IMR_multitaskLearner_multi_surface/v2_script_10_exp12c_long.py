if __name__=='__main__':
    ...
    def get_model_invariant(train_loader1, train_loader2, len_embedding, abstract_len_embedding):
        ...
        for epoch in range(num_iterations):
            ...
            if epoch%1==0:
                with open(curdir + 'v2_script_10/exp12/logger_' + exp_name + '.log', 'a+') as file1:
    ...
    train_data1 = TensorDataset(torch.tensor(X1, dtype=torch.float32), torch.tensor(y1, dtype=torch.float32))
    train_data2 = TensorDataset(torch.tensor(X2, dtype=torch.float32), torch.tensor(y2, dtype=torch.float32))
