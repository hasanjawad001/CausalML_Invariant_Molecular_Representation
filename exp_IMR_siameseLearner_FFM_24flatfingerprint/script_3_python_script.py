...
#### 1. seperate feature target for 6 envs 

dir_datasets = curdir + 'datasets/' 

df_1_smiles = pd.read_csv(dir_datasets + 'smiles.csv', header=None)
m_1_smiles = df_1_smiles.values[:,[2]]
# print(m_1_smiles.shape)
...
# print(m_1_energy.shape, m_1_name.shape, m_1_smiles.shape, m_1_energy_envs.shape)
...
def get_embed_from_smiles(list_species_smiles, emb_style='auto_768'):
    if emb_style=='manual_24':
        ...
        res = np.array(list_emb)[:,:-2]
        print('check check check: ', res.shape)
        return res
...
for env_no in range(4): 
    ...
    array_emb = get_embed_from_smiles(smiles, emb_style='manual_24') ## change_2 : change in v10(24) from v9(768) to accomodate manual 24 length embedding 
    print(np.sum(array_emb))
    feature_target = np.hstack((array_emb, m[:,[2]])) ## index 2 used here for energy
    
    print(feature_target.shape)
    np.savetxt(dir_datasets + 'embedding_vs_energy_env_' + str(env_no) + '.csv', feature_target, delimiter = ",", fmt='%s')    
#     print()
...
## skip invalid samples => index in each env ['-','-','-','-','-','-'] for morgan_24
...
for env_no in range(4): 
    ...
    print(feature_target.shape)
    np.savetxt(dir_datasets + 'filtered_embedding_vs_energy_env_' + str(env_no) + '.csv', feature_target, delimiter = ",", fmt='%s')
...
#
m_1_energy = df_1_energy.values[1:]
m_1_feature = df_1_feature[:,list(range(0,24))]
...
#     print(len(senvironments), senvironments[0][0].shape, senvironments[0][1].shape)  
    ...
    #-----------change_1    
    for _n_hidden_node in [128]: ## [512,256,128,64,32,16,8] ## hint: select something that is divisible by 4  
    #-----------change_1
        for _lr in [1e-4, 1e-3, 1e-2, 1e-1, 1]: ## [1e-4, 1e-3, 1e-2, 1e-1, 1] 
            for _struct_code in [1]: ## [0,1]
...
def get_result(
    ...
    for env_no in list_env:
        ...
        var_trial = trial_no
        var_env_no = env_no
        var_nhn = str(best_model_info[0])
        var_lr = str(best_model_info[1]), 
        var_struct_code = str(best_model_info[2])
        var_model = best_model_info[3] 
        # Save the model
        model_path = curdir + 'datasets/best_model_' + str(var_trial) + '_' + str(var_env_no) + '_' + str(var_nhn) + '_' + str(var_lr) + '_' + str(var_struct_code) + '.pt'
        torch.save(var_model.state_dict(), model_path)
        #######################################################################
