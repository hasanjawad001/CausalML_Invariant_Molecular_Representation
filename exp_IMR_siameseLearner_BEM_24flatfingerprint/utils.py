    ...
    def Ridge_CV(self, trainX, testX, trainY, testY):
        ...
        for a in alpha_vals:        
            ...
            for train_indices, validation_indices in kf.split(trainX):
                training_set_X, validation_set_X = trainX[train_indices], trainX[validation_indices]
                training_set_Y, validation_set_Y = trainY[train_indices], trainY[validation_indices]
                regr = Ridge(alpha=a, tol=self.tol)
                regr.fit(training_set_X, training_set_Y)
                predY = regr.predict(validation_set_X)
                errorY = np.absolute(predY - validation_set_Y)
                errors[j] = np.mean(errorY)
    ...
    def Lasso_CV(self, trainX, testX, trainY, testY):
        ...
        for a in alpha_vals:        
            ...
            for train_indices, validation_indices in kf.split(trainX):
                training_set_X, validation_set_X = trainX[train_indices], trainX[validation_indices]
                training_set_Y, validation_set_Y = trainY[train_indices], trainY[validation_indices]
                regr = Lasso(alpha=a, tol=self.tol)
                regr.fit(training_set_X, training_set_Y)
                predY = regr.predict(validation_set_X)
                errorY = np.absolute(predY - validation_set_Y)
                errors[j] = np.mean(errorY)
        ...
        for a in alpha_vals:    
            for l in l1ratio_vals:
                ...
                for train_indices, validation_indices in kf.split(trainX):
                    training_set_X, validation_set_X = trainX[train_indices], trainX[validation_indices]
                    training_set_Y, validation_set_Y = trainY[train_indices], trainY[validation_indices]
                    regr = linear_model.ElasticNet(alpha=a, l1_ratio=l, tol=self.tol)
                    regr.fit(training_set_X, training_set_Y)
                    predY = regr.predict(validation_set_X)
                    errorY = np.absolute(predY - validation_set_Y)
                    errors[j] = np.mean(errorY)
...
def ywq_readSmilesToFingerprints(smstr):
    l = len(smstr)
    i = 0
    prevatom = None
    prevnegval = None
...
#            vec[2] += 1 #number of O
        ...
        if i == 0 and (smstr[i] == 'C' or smstr[i] == '['):
            termialCarbon = True
        ...
        if i == len(smstr)-1 and (smstr[i] == 'C' or smstr[i] == ']'):
            termialCarbon = True
#        elif smstr[i] == 'O':
#            atom = smstr[i]
#            negval = 0
...
#            vec[o_zero_pos_at_vec + negval] += 1 #number of O0 or O1; starting at vector index 3
            rem_positions -= negval
            if termialCarbon:
                if doubleBond == 0 and tripleBond == 0:
                    rem_positions -= 1
                else:
                    rem_positions -= (doubleBond*2 + tripleBond*3)
            else:
                if doubleBond == 2:
                    rem_positions -= (doubleBond*2)
                elif doubleBond == 1:
                    rem_positions -= (doubleBond*2 + 1)
                elif tripleBond== 1:
                    rem_positions -= (1 + tripleBond*3)
                else:
                    rem_positions -= 2
...
#            vec[13] += rem_positions #number of O-H
        ...
        if prevatom is not None:
            ...
            bondmatrix[rowstartidx + prevnegval, colstartidx + negval] += 1
        i += 1
        print('loop {}, atom {}, num of H atoms {}, charge {}, terC {}'.format(i, atom, rem_positions, negval, termialCarbon))
        
    for j in range(3,6):
        for i in range(2,j):
            bondmatrix[i,j] += bondmatrix[j,i]
    vec[14:-2] = (bondmatrix[2:,2:])[np.triu_indices(4)]
    vec[-1] = smstr.count('#')
    vec[-2] = smstr.count('=')
#    vec[10] += np.sum(bondmatrix[2:,0]) + np.sum(bondmatrix[0,2:])
#    vec[11] += np.sum(bondmatrix[2:,1]) + np.sum(bondmatrix[1,2:])
    #redfine the single C
    if vec[1] == 1:
        vec[0] = 4-negval
        vec[9] = vec[0]
    return vec
